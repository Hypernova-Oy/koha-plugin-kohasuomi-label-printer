package Koha::Plugin::Fi::KohaSuomi::LabelPrinter::DataSourceFormatter;
...
@PARAM2 Koha::Plugin::Fi::KohaSuomi::LabelPrinter::Sheet::Element, the Element-object containing all the formatting parameters
...
sub public_barcode39 {
    ...
    $log->debug("Barcode39 x=".$pos->{x}.','."y=".$pos->{y}." ".$text) if $log->is_debug;
...
sub public_barcode128 {
    ...
    $log->debug("Barcode128 x=".$pos->{x}.','."y=".$pos->{y}." ".$text) if $log->is_debug;
...
sub public_barcodeEAN13 {
    ...
    if ($text =~/[^0-9]/) {
        my @cc = caller(0);
        Koha::Exceptions::BadParameter->throw(error => $cc[3]."():> Given text '$text' has unallowed characters. Only these characters are allowed: 0-9");
    }
    if (length($text) != 13) {
        my @cc = caller(0);
        Koha::Exceptions::BadParameter->throw(error => $cc[3]."():> Given text '$text' must be 13 characters long.");
    }
    ...
    $log->debug("EAN13 x=".$pos->{x}.','."y=".$pos->{y}." ".$text) if $log->is_debug;
...
sub public_barcodeEAN13checksum {
    ...
    if (length($text) != 12) {
        my @cc = caller(0);
        Koha::Exceptions::BadParameter->throw(error => $cc[3]."():> Given text '$text' must be 12 characters long.");
    }
    ...
    $log->debug("EAN13checksum x=".$pos->{x}.','."y=".$pos->{y}." ".$text) if $log->is_debug;
    PDF::Reuse::Barcode::EAN13  (%pos,
                                 value         => $text,
                                 text          => $showText,);
}
...
sub _printLines {
    ...
    for (my $i=0 ; $i<scalar(@$lines) ; $i++) {
        ...
        my @pos = ($pos->{x}, $posTop);
        $log->debug("_printLine x=".$pos->{x}.','."y=".$posTop." ".$line) if $log->is_debug;
