package Koha::Plugin::Fi::KohaSuomi::LabelPrinter::DataSourceSelector;
...
@PARAM2 ARRAYRef of HASHRefs, source data. See Koha::Plugin::Fi::KohaSuomi::LabelPrinter::DataSource for more information
...
sub _evalSegment {
    ...
    foreach my $op (@$or) {
        if (my $marcSel = _isMARCSelector($op)) {
            ...
            my @cc = caller(0);
            Koha::Exceptions::BadParameter->throw(error => $cc[3]."($op):> Couldn't parse this source definition '$op'");
        }
    }
    return \@payload;
}
...
sub _getDBSelectorValue {
    ...
    unless ($table) {
        my @cc = caller(0);
        Koha::Exceptions::BadParameter->throw(error => $cc[3]."():> data source requests table '".$selector->{table}."', but that table is not available. Add more database sources in Koha::Plugin::Fi::KohaSuomi::LabelPrinter::DataSourceManager::_getDataSourceParams()");
    }
    unless (exists($table->{ $selector->{column} })) {
        my @cc = caller(0);
        Koha::Exceptions::BadParameter->throw(error => $cc[3]."():> data source requests table '".$selector->{table}."' and column '".$selector->{column}."', but that column is not available. Add more database sources in Koha::Plugin::Fi::KohaSuomi::LabelPrinter::DataSourceManager::_getDataSourceParams()");
    }
