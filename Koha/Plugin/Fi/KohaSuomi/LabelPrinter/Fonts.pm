package Koha::Plugin::Fi::KohaSuomi::LabelPrinter::Fonts;
...
=head getFont
    ...
    my $font = Koha::Plugin::Fi::KohaSuomi::LabelPrinter::Fonts::getFont($fontName);
...
sub getFont {
    ...
    if ( -e $ttf_path->{content} ) {
        ...
        my @cc = caller(0);
        Koha::Exceptions::ObjectNotFound->throw(error => $cc[3]."($font):> No such font defined in koha-conf.xml <font type=\"$font\">/path/to/font.ttf</font>");
    }
}
...
sub _getFontsSyspref {
    my $ttf = C4::Context->config('ttf') or Koha::Exceptions::Config::MissingEntry->throw(error => __PACKAGE__.":: TrueType-fonts not configured in \$KOHA_CONF!");
    return $ttf->{font};
}
