package Koha::Plugin::Fi::KohaSuomi::LabelPrinter::Sheet::Item;
...
sub setIndex {
    my ($self, $index) = @_;
    unless ($index =~ /^\d+$/) {
        Koha::Exceptions::BadParameter->throw(error => __PACKAGE__.":: Parameter 'index' is missing or is not a digit");
    }
    $self->{index} = $index;
}
...
sub setRegions {
    ...
    unless (ref($regions) eq 'ARRAY') {
        Koha::Exceptions::BadParameter->throw(error => __PACKAGE__.":: Parameter 'regions' is not an array");
    }
    $self->{regions} = [];
    foreach my $region (@$regions) {
        push(@{$self->{regions}}, Koha::Plugin::Fi::KohaSuomi::LabelPrinter::Sheet::Region->new($self, $region));
    }
}
...
sub setParent {
    ...
    unless (blessed($sheet) && $sheet->isa('Koha::Plugin::Fi::KohaSuomi::LabelPrinter::Sheet')) {
        Koha::Exceptions::BadParameter->throw(error => __PACKAGE__.":: Parameter 'parent' is not a Sheet-object");
    }
    $self->{parent} = $sheet;
}
